...
function validateForm() {
	...
	let tableName = document.getElementById('productType').value;
    console.log("this is tableName form  validateForm(tableName): ", tableName);
    
        //Check if SKU, Name, and Price are not empty
    ...
    // // Perform asynchronous SKU validation
    validateSKU(sku, tableName);

    // Return false for now, the actual form submission should happen after validation
    return false;
...
function validateSKU(sku, tableName) {
        console.log("SKU:", sku);
    ...
    xhr.send("sku=" + encodeURIComponent(sku) + "&table=" + encodeURIComponent(tableName));
}
